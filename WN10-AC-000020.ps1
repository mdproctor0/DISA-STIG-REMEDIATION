<#
.SYNOPSIS
    This PowerShell script configures the password history setting to remember at least
    24 previous passwords to prevent password reuse.
...
# ------------------------------------------------------------
# This means:
# "Remember the last 24 passwords so users can't reuse them."
...
try {
    ...
    # --------------------------------------------------------
    # STEP 4: Update PasswordHistorySize value
    # --------------------------------------------------------
    ...
    $UpdatedLines = foreach ($line in $CfgLines) {
        if ($line -match '^\s*PasswordHistorySize\s*=') {
            $Found = $true
            "PasswordHistorySize = $RequiredHistory"
        }
        else {
            $line
        }
    ...
    if (-not $Found) {
        ...
        foreach ($line in $CfgLines) {
            ...
            if ($line -match '^\[System Access\]\s*$' -and -not $Inserted) {
                $UpdatedLines += "PasswordHistorySize = $RequiredHistory"
                $Inserted = $true
            }
        }
    ...
    $CurrentLine = $VerifyLines | Where-Object { $_ -match '^\s*PasswordHistorySize\s*=' } | Select-Object -First 1

    if (-not $CurrentLine) {
        Write-Output "FAIL: Could not find PasswordHistorySize in policy."
        exit 2
    }

    $CurrentValue = [int]($CurrentLine -split '=')[1].Trim()

    if ($CurrentValue -ge $RequiredHistory) {
        Write-Output "PASS: Password history is $CurrentValue passwords (Required: >= $RequiredHistory)."
        exit 0
    }
    else {
        Write-Output "FAIL: Password history is $CurrentValue passwords (Required: >= $RequiredHistory)."
        exit 2
    }
}
catch {
